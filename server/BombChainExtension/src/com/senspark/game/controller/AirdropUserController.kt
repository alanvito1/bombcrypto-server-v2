...
import com.senspark.game.manager.MasterAirdropUserManager
...
class AirdropUserController(
    ...
    override val svServices = _services.get<ISvServicesContainer>().get(ServerType.from(dataType))
    ...
    private val _needSave = EnumConstants.SAVE.entries.associateWith { false }.toMutableMap()
    ...
    override fun initDependencies(): Boolean {
        ...
        _isDisposed = false
        try {
            masterUserManager = MasterAirdropUserManager(_userPermissions, createMediator(), _verifyAdApi)
    ...
    private fun saveReward() {
        try {
            ...
            val mapMiningFiReward = userRw.getRewardsMiningFi()[_userInfo.dataType]?.toMutableMap() ?: mutableMapOf()
            ...
            if (mapReward.isNotEmpty()) {
                ...
                for (reward in mapReward.entries) {
                    ...
                    // lưu ranking cho user TON
                    if (rewardDetail.blockRewardType == EnumConstants.BLOCK_REWARD_TYPE.COIN) {
                        _coinRanking.saveRankingCoin(userId, rewardDetail.value, dataType)
                        if (dataType == EnumConstants.DataType.TON) {
                            _referralManager.addEarning(userId, rewardDetail.value.toDouble())
                            val coinSeason = _coinRanking.currentSeasonNumber
                            _clubManager.addMemberPoint(userId, rewardDetail.value.toDouble(), coinSeason)
                        }
                    }
                }


                // Lưu database cho star core network
                for (reward in mapMiningFiReward.entries) {
                    val rewardDetail = reward.value
                    _rewardDataAccess.addUserBlockReward(
                        userId,
                        rewardDetail.blockRewardType,
    ...
    var isReceivedOfflineTHMode = false
    
    private val _coinRanking = svServices.get<ICoinRankingManager>()
    private val _referralManager = svServices.get<IReferralManager>()
    private val _clubManager = svServices.get<IClubManager>()
}